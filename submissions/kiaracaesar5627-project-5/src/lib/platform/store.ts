...
import { computeMetrics, isBlockedUserId } from "./qualify";
import type { LearningEventName, StoredEvent } from "./qualify";
import { loadPersistedEvents, persistMode, savePersistedEvents } from "./persist";

export type { LearningEventName, StoredEvent } from "./qualify";
...
export const SEEDED_APP = {
  ...
  repo_url: `https://github.com/rogerSuperBuilderAlpha/hult-cohort-program/tree/participants/summer26/phase-2-venture/${SITE.handle}/submissions/${SITE.handle}-project-5`,
...
export function isBlockedUser(userId: string, studentHandle: string): boolean {
  ...
  if (getStore().blockedUserIds.has(normalizedUser)) return true;
  return isBlockedUserId(userId, studentHandle);
}
...
export function recordEvent(
  ...
  if (isBlockedUser(payload.user_id, SEEDED_APP.student_handle)) return;
  const store = getStore();
  store.events.push({
    app_id: appId,
    event: payload.event,
  ...
  savePersistedEvents(store.events);
}
...
export function getMetrics(appId: string): {
  ...
  as_of: string;
  source: string;
  persist_mode: ReturnType<typeof persistMode>;
} {
  const counts = computeMetrics(getStore().events, appId);
  return {
    ...counts,
